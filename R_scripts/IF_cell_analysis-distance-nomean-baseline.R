...
distance |>
  ggplot(aes(x=group,
             y=(distance_to_annotation_with_baseline_um+0.1),
             color=group))+
  geom_boxplot(outlier.shape = NA)+
  geom_point(position = position_jitterdodge())+
  theme_pubr()+
...
ggplot(data = glans, aes(x = distance_to_annotation_with_epithelium_um)) +
  geom_freqpoly()

model_glans <- lme4::lmer(log10(distance_to_annotation_with_baseline_um) ~ condition*group*layer*target_cell+(1|donor), 
                          data = glans)
...
# cells in uncircumcised baseline between infected and non infected-- a significant difference was not detected in corresponding circumsized tissue:
...
distance |> 
  filter(tissue == "Glans") |>
  ggplot(aes(x = group, y = distance_to_annotation_with_baseline_um, color = group)) +
  geom_boxplot(outlier.shape = NA) +
  geom_point(position = position_jitterdodge()) +
  ...
  labs(title = "Glans Samples",
       y = expression ("Distance to annotation baseline in "~mu*m)) +
  rotate_x_text(45)
...
model_shaft <- lme4::lmer(log10(distance_to_annotation_with_baseline_um) ~ condition*group*layer*target_cell+(1|donor), data = shaft)
